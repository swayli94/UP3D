...
def test_g23_cl_vs_panel_reference(medium_case, coarse_case,
                                   reference_mesh_dir, gate_evidence_dir):
...
def test_g24_gamma_vs_pressure_lift(medium_case, gate_evidence_dir):
    ...
    gate_dir = gate_evidence_dir / "G2.4"
    gate_dir.mkdir(parents=True, exist_ok=True)
    with open(gate_dir / "summary.csv", "w", newline="") as f:
        w = csv.writer(f)
...
@pytest.mark.skipif(not gate_figures_enabled(),
                    reason="C/D 证据图只在 PYFP3D_GATE_FIGURES=1 时写盘 —— 平时不写，避免每次 pytest 都脏工作树（2026-08-24）")
def test_g25_quasi2d_consistency(coarse_case, medium_case, gate_evidence_dir):
    ...
    gate_dir = gate_evidence_dir / "G2.5"
    gate_dir.mkdir(parents=True, exist_ok=True)
    with open(gate_dir / "summary.csv", "w", newline="") as f:
        csv.writer(f).writerows(rows)
...
@pytest.mark.skipif(not gate_figures_enabled(),
                    reason="C/D 证据图只在 PYFP3D_GATE_FIGURES=1 时写盘 —— 平时不写，避免每次 pytest 都脏工作树（2026-08-24）")
def test_v21_v22_cut_artifacts(coarse_case, gate_evidence_dir):
    ...
    gate_dir = gate_evidence_dir / "G2.1"
    gate_dir.mkdir(parents=True, exist_ok=True)
    ...
    gate_dir = gate_evidence_dir / "G2.2"
    gate_dir.mkdir(parents=True, exist_ok=True)
    with open(gate_dir / "summary.csv", "w", newline="") as f:
        w = csv.writer(f)
...
@pytest.mark.skipif(not gate_figures_enabled(),
                    reason="C/D 证据图只在 PYFP3D_GATE_FIGURES=1 时写盘 —— 平时不写，避免每次 pytest 都脏工作树（2026-08-24）")
def test_v23_cp_curves_vs_panel(medium_case, reference_mesh_dir, gate_evidence_dir):
