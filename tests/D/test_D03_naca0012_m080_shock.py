...
★★ 2026-08-24：`artifacts/` 已删除，C/D 的图改写进**被跟踪的** `cases/gates/<门号>/`（`PYFP3D_GATE_FIGURES=1` 时刷新）⇒ 「因为 artifacts/ 是 gitignored 所以不画图」这个理由**不再成立**，本门可以恢复出图。
"""
...
def _write_g41_summary(case, level, gate_evidence_dir):
    ...
    gate_dir = gate_evidence_dir / "G4.1"
...
def test_g43_robustness_sweep(gate_evidence_dir):
    ...
    gate_dir = gate_evidence_dir / "G4.3"
    gate_dir.mkdir(parents=True, exist_ok=True)
