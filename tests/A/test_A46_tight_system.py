...
coarse smoke Newton (binding: phases/p1/docs/roadmap/track_v.md GV5.1 + the
...
3.0e6 -- the GV3.1 configuration, coarse recorded):
  ...
  - the FULL-SYSTEM FD gate: 4 random max-normalized full-state
    ...
    (tests/A/test_A45_tight_edge.py:205-219; zero at the k=1 state), masked
...
JIT-lane only, the tests/A/test_A42_loose_coupling.py:204 precedent; the cheap
...
def _fallback_row_mask(st):
    """The Stage-2 mask (tests/A/test_A45_tight_edge.py:205-219): drop the 6
