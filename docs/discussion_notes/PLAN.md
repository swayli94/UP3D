...
> - `DN2 §x` → `design_notes/20260707_2118_ibl_viscous_coupling_design.md` §x
> - `DN1 §x` → `design_notes/20260707_1505_levelset_wake_design.md` §x
> - `DN4 §x` → `design_notes/20260708_1646_newton_solver_strategy.md` §x
> - `DN5 §x` → `design_notes/20260708_1744_implementation_audit.md` §x
> - `DN6 §x` → `design_notes/20260709_0145_3d_vii_implementation_analysis.md` §x
...
Track P:  P0 ✓ → P1 △ → P2 ✓ → P3 ✓ → P4 ✓ → P5 ✓ → P6 ✓ → P7 ☐ → P8 ☐ → P9 ☐ → P10 ☐
                                                                    ↑
Track B:  (B1 → B2 → B3 → B4 → B5)   [B6 搁置]                      P8 依赖 B3
                                                                    ↑
...
**测试基线**：157 passed + 2 xfailed (HEAD after P6)
...
| **目标** | 推导 + FD 验证 ∂ρ̃/∂φ for the shipped walk flux at frozen upstream selection |
| **关键认识** | Newton 前置**不是**新通量——P4 walk 在 frozen u(e) 下已 C¹（isentropic ρ + branch-wise ∂ν/∂φ），只有两个 `max` kink。López 保留 C⁰ switch 仍二次收敛。`max_ε` 仅 optional robustness |
| **交付物** | N1: ∂ρ̃/∂φ 推导 + FD 验证; N2 (P8 中): 可选 streamline-kernel Jacobian 对比 |
| **Gate** | G7.1 亚临界 no-op; G7.2 解保持; G7.3 FD-verify ∂ρ̃/∂φ |
...
| **验证** | Newton 二次收敛; 2D NACA 0012 跨声速 VII 修正激波位置 vs 实验; 3D ONERA M6 CL 从 0.245 → 0.288 |
...
**可并行路径**：
- Track B (B1–B5) 可与 P7–P8 并行开发
- V1 可在 P6 完成后独立开始（不依赖 P7–P8）
- P9 可与 P7–P8 并行
