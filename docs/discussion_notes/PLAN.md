...
| **Track B** — Level-Set 尾流 | 嵌入式尾流替代 conforming 网格尾流 | **进行中**：B1 ✓ (2026-07-11)；B2–B5（含新 gate B3.5/B4.5）待开发；数值设计参考已升级为 [design_track_b.md](../design_track_b.md)（取代 DN1） |
...
Track P:  P0 ✓ → P1 △ → P2 ✓ → P3 ✓ → P4 ✓ → P5 ✓ → P6 ✓ → P7 ✓ → P8 ✓ → P9 ◐ → P10 ☐ → P11 ☐ → P12 ☐
                                                                    ↑
Track B:  B1 ✓ → B2 ☐ → B3 → B3.5 → B4 → B4.5 → B5   [B6 搁置]   P8 Newton 已先在 conforming
                                                                  尾流上落地；level-set Newton = B4 后重推导
...
| M4 | ✅ | 2026-07-11 | 无尾迹面 ONERA M6 族（Track B 3D 用；复用 M1 走廊尺寸场，单元数差 6–9% ⇒ B4.5 受控 A/B） |
| P0 | ✅ | 2026-07-06 | 脚手架 |
| P1 | △ | — | G1.6 球 Cp 待 P9 |
...
| B2–B5 | ☐ | — | Level-Set 尾流（含 2026-07-11 新增 B3.5 远场 A/B、B4.5 M6 3D gate；设计参考 design_track_b.md） |
| B6 | ⏸ | — | 曲线尾流（搁置） |
| V1 | ☐ | — | IBL3 松耦合 |
...
| **网格族** | M3: 29.3k/147.7k tets（走廊楔形代价 ~1.8–2.4× M0）；M4: 50.6k/329.6k（与 M1 差 6–9% ⇒ B4.5 受控 A/B） |
...
> **状态**：**进行中** —— B1 ✓ (2026-07-11)；双网格规则（嵌入面网格 ε 压力测试+严格 A/B vs 无尾迹 M3 一般切割）适用于 B1–B4 每个 gate。

| 阶段 | 内容 | 状态 | 依赖 | 设计参考 |
|------|------|------|------|-------------|
| **B1** | Level-set 尾流 + 切单元识别（验证基础设施） | ✅ 2026-07-11 | — | design_track_b.md §3/D4/D9 |
| **B2** | 多值 FE 装配（核心数值验证） | ☐ | B1 | design_track_b.md §2.1/§2.5/§5 |
| **B3** | **隐式 Kutta**（TE 复制+尾迹 LS;罚函数降为可选诊断）+ 升力求解 | ☐ | B2 | design_track_b.md §2.2–2.3/D1/D2 |
| **B3.5** | 远场 a/b 实测对比（新 2026-07-11） | ☐ | B3 | design_track_b.md §5.4 |
| **B4** | 跨声速 + Mach continuation（重锚:coarse M0.80 G8.1 锁带、medium M0.7875） | ☐ | B3 | design_track_b.md §7 |
| **B4.5** | M6 3D gate（新 2026-07-11:TE 折线直纹面/g₂ 展向留空/尖端 Γ→0） | ☐ | B4 | design_track_b.md §7 |
| **B5** | 多尾流验证（多段翼/翼身组合体） | ☐ | B4.5 | DN1 §8 Phase B5 |
...
**Track B 与 Track P 的关系**：B1–B4 完成后可以替代当前 conforming 尾流路径；Track B 不阻塞 Track P 的 P7–P10（当前 conforming 尾流继续工作）。

**时序护栏（2026-07-10 记录;P8 已于 2026-07-11 关闭,前半兑现）**：P8 全耦合 Newton 的 Γ-Jacobian 块建立在 master-slave 消元上（`wake.py::self._h`），而 B3 隐式 Kutta 取消 Γ 自由度——两套 Newton 设计不并行推进。P8 已先在 conforming 尾流上落地；level-set 路径的 Newton 是 B4 之后的重推导（更简:尾迹 LS 块在 φ 中线性/常数,无 Γ 消元,见 design_track_b.md §5.5）。
