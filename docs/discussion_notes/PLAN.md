...
>
> **⚠ Track-P 重编号（2026-07-11，用户指示，同日两次插入）**：新增 **P9 = 网格收敛/
> 精度缺口判别**（证据阶段：G9.1 M6 三点 Richardson 外推、G9.2 2.5D 尖后缘升力
> oracle、G9.3 归因裁决）与 **P10 = Newton 通用性与续接效率**（G10.1 非升力入口、
> G10.2 层级自适应容差）。净映射：原 P9（曲面壁面单元）→ **P11**、原 P10（Backlog）
> → **P12**。本文件下文各节仍用旧编号（"P9 曲面单元" 应读作 P11，"P10 Backlog"
> 应读作 P12）；细节以 roadmap.md 为准。
...
| 开发线 | 目标 | 当前状态 |
|--------|------|---------|
| **Track M** — 网格生成 | 为求解器提供非结构网格，支持后续复杂几何 | M0/M1/M3 已关闭；M2 待开发 |
| **Track P** — 势流求解器 | FPE 的准确、高效求解 | P0–P8 已关闭；P9（判别）进行中；P10–P12 待开发 |
| **Track B** — Level-Set 尾流 | 嵌入式尾流替代 conforming 网格尾流 | **进行中**：**B1–B5 ✓**（B1 2026-07-11；B2/B3/B4/B5 2026-07-12）；下一步 B6（跨声速）/ B7（M6 3D）；数值设计参考 [design_track_b.md](../design_track_b.md)（取代 DN1） |
| **Track V** — 粘性耦合 (VII) | IBL3 6 方程 + transpiration 耦合 | V1–V4 全部待开发（设计完成） |

**依赖关系**：

```
Track M:  M0 ✓ → M1 ✓ → M2 ☐ ;  M3 ✓ (无尾迹族, 供 Track B 双网格规则)
                      Ↄ (为 P/B/V 提供网格)
Track P:  P0 ✓ → P1 △ → P2 ✓ → P3 ✓ → P4 ✓ → P5 ✓ → P6 ✓ → P7 ✓ → P8 ✓ → P9 ◐ → P10 ☐ → P11 ☐ → P12 ☐
                                                                    ↑
Track B:  B1 ✓ → B2 ✓ → B3 ✓ → B4 ✓ → B5 ✓ → B6 ◐ → B7 ✓ → B8 ☐(LS翼尖taper) → B9 ☐(多尾流)   [B10 曲线尾流搁置]
          (B4 = TE 控制体/隐式 Kutta, 2026-07-12 新插入; B5 = 远场 A/B)   P8 Newton 已先在
          conforming 尾流上落地；level-set Newton = post-B6 重推导
...
| M2 | ☐ | — | 翼身组合体尾流-机身交接 |
| M3 | ✅ | 2026-07-11 | 无尾迹面 NACA 准 2D 族（Track B 双网格规则用；走廊扇形尺寸场覆盖 α 扫掠） |
| M4 | ✅ | 2026-07-11 | 无尾迹面 ONERA M6 族（Track B 3D 用；复用 M1 走廊尺寸场，单元数差 6–9% ⇒ B7 受控 A/B） |
| P0 | ✅ | 2026-07-06 | 脚手架 |
| P1 | △ | — | G1.6 球 Cp 待 P9 |
...
| P8 | ✓ | 2026-07-11 | 全耦合 Newton（G8.1 二次收敛；G8.2 M6 medium 249 s；G8.3 套件 5m02s；P4/P5 Picard 收敛质量勘误见 roadmap） |
| P9 | ✓ | 2026-07-11 | 网格收敛/精度缺口判别。**结论:0.019 缺口按原命题不可分解**——G9.2 通过(2D 尖尾误差 2.71%→0.33%→0.03%,**尖尾不构成升力底板**);G9.1 无效(M6 fine **不是离散解**:未限速 M_max 1.40→2.13→**7.93**,9 个单元越过 M_cap=3,永久限速使 freeze 机制无法启动 → 极限环 ~1e-5,其 cl_KJ 0.2393 是伪值)。**★ 发散奇异性位于尾涡面的自由翼尖边缘**(9 个越界单元:z/b 0.998–1.000、x−x_TE +0.002…+0.017 即**在后缘之后**、|y|<0.003 弦平面内),正是强制 Γ(tip)=0 之处——刚性平面尾涡的**涡面边缘奇异性**(真实流动卷成翼尖涡)。P5 记录的"有界翼尖后缘过冲"(M_max 1.995)是同一对象,只是当时网格不够细。**⇒ 这是尾涡模型缺陷(正是 Track B 要解决的),不是壁面单元缺陷** → **P11 的 3D 升力立论不成立**,其余有效理由只剩 G1.6(光滑曲面壁,机理不同)。用户待仲裁。 |
| P10 | ◐ | — | Newton 通用性与续接效率（新 2026-07-11）：G10.2 已关闭（2026-07-11，A/B 分裂裁决：M6 medium +41.4% 锁全过 → intermediate_tol=1e-5 升入 NEWTON_M6_RECIPE；NACA 折叠区负结果 → NEWTON_TRANSONIC_RECIPE 保持严格，折叠区禁用宽松中间层）；G10.3 已关闭（2026-07-11，裁决=保留 ramp：远折叠区双种子均 A 类同解——分支选择非约束，但轻种子瞬态穿越 clamp 态（+44% 却违反预注册无-clamp 条款）、深种子无 clamp 但仅 +6.6% 不达 ≥20%；折叠区 medium 双种子均 C 类，深 Picard 种子无续接时自身发散——"+44% 但有 clamp 瞬态"留用户仲裁）；G10.1 非升力入口待开发 |
| P11 | ☐ | — | 曲面壁面单元（原 P9；是否开启视 G9.3 裁决） |
| P12 | ☐ | — | Backlog（原 P10） |
| B1 | ✅ | 2026-07-11 | level-set 尾流 + 切单元识别；2.5D census 与 conforming minus-star 逐单元一致、3D(M6) 为严格超集（+2.9% 翼尖边跨越单元）；★3D 才暴露：后掠 TE 须用斜交展向坐标 + 展向裁剪（翼尖外不切） |
| B2 | ✅ | 2026-07-11 | 多值 FE 装配（main→aux 列重定向；weld 闭合精确退化为单值，1e-13） |
| B3 | ✅ | 2026-07-12 | 隐式 Kutta 升力求解（Γ 是结果，无 secant/无 master-slave）；cl_KJ 0.2828 落在 [PG,KT] 内 |
| B4 | ✅ | 2026-07-12 | **新插入** TE 控制体 / 隐式 Kutta 再推导：★尾迹 LS 对常数跳跃恒零（1.9e-16）⇒ Γ 需独立方程；非线性 TE 压力相等 Kutta + **壁面邻接**控制体（全扇形 +11~15%，壁面邻接 <1%） |
| B5 | ✅ | 2026-07-12 | 远场 A/B（旧 B3.5→B4.5）：选项 a（Dirichlet+涡）保持默认；选项 b（López Neumann）截断 O(Γ/R)，需 R≥30c |
| B6 | ☐ | — | 跨声速 + Mach continuation（旧 B4） |
| B7 | ☐ | — | M6 3D gate（旧 B4.5） |
| B8 | ☐ | — | 多尾流验证 / 翼身组合体（旧 B5） |
| B9 | ⏸ | — | 曲线尾流（搁置；旧 B6） |
| V1 | ☐ | — | IBL3 松耦合 |
...
### M3 / M4 — 无尾迹面网格族（Track B 双网格规则）

| 项 | 内容 |
|---|---|
| **状态** | ✅ 已关闭 (2026-07-11，用户指示) |
| **roadmap** | `roadmap Track M` (M3, M4) |
| **交付物** | `meshgen/planar.py embed_wake=False`（M3，NACA 准 2D + ±6° 走廊扇形尺寸场）、`meshgen/wing3d.py embed_wake=False`（M4，ONERA M6，弦平面片只喂尺寸场，不 fragment/embed）；生成脚本 `cases/meshes/naca0012_wakefree_2.5d/`、`cases/meshes/onera_m6_wakefree/` |
| **网格族** | M3: 29.3k/147.7k tets（走廊楔形代价 ~1.8–2.4× M0）；M4: 50.6k/329.6k（与 M1 差 6–9% ⇒ B7 受控 A/B） |
| **要点** | 网格拓扑完全不知道尾迹存在（无 `wake` tag）——Track B 的真实交付形态；3D 不做 α 楔形走廊（楔体积随展长，代价不成比例），宽包络扫掠演示放在 2.5D M3 |
...
> **设计文档**：[design_track_b.md](../design_track_b.md)（2026-07-11,López 论文逐条对照,**取代 DN1** 作为数值设计参考;DN1 保留为方案比选历史记录）
> **动机（2026-07-11 用户裁决重估）**：定位 = 网格/几何**工作流能力**（免嵌入尾迹面、α 扫掠不重网格、M2 翼身、钝尾缘、消 st133 类探针故障），**不是**求解速度（消 Γ-secant 的效率动机已被 P8 Newton 淘汰）。
> **状态**：**进行中** —— **B1–B5 ✓**（B1 2026-07-11；B2/B3/B4/B5 2026-07-12），下一步 B6/B7；双网格规则（嵌入面网格 ε 压力测试+严格 A/B vs 无尾迹 M3 一般切割）适用于 B1–B6 每个 gate。
> **⚠️ 2026-07-12 两次 Track-B 重编号**：新插入 B4（TE 控制体），随后取消半整数编号。净映射：远场 A/B（曾 B3.5→B4.5）→ **B5**；旧 B4 跨声速 → **B6**；旧 B4.5 M6 3D → **B7**；旧 B5 多尾流 → **B8**；旧 B6 曲线尾流 → **B9**。**⚠️ 2026-07-13 再次重编号**：新插入 **B8 = level-set 翼尖去奇异（row-blend tip taper）**，其后后移一位——多尾流 **B8→B9**、曲线尾流 **B9→B10**。本文下方表格 B8/B9 为旧编号。以 roadmap 为准。

| 阶段 | 内容 | 状态 | 依赖 | 设计参考 |
|------|------|------|------|-------------|
| **B1** | Level-set 尾流 + 切单元识别（验证基础设施） | ✅ 2026-07-11 | — | design_track_b.md §3/D4/D9 |
| **B2** | 多值 FE 装配（核心数值验证） | ✅ 2026-07-11 | B1 | design_track_b.md §2.1/§2.5/§5 |
| **B3** | **隐式 Kutta**（TE 复制+尾迹 LS）+ 升力求解 | ✅ 2026-07-12 | B2 | design_track_b.md §2.2–2.3/D1/D2 |
| **B4** | **TE 控制体 / 隐式 Kutta 再推导**（2026-07-12 新插入）：非线性 TE 压力相等 Kutta + 壁面邻接控制体 | ✅ 2026-07-12 | B3 | design_track_b.md §9 |
| **B5** | 远场 a/b 实测对比（旧 B3.5→B4.5） | ✅ 2026-07-12 | B3 | design_track_b.md §5.4 |
| **B6** | 跨声速 + Mach continuation（旧 B4;重锚:coarse M0.80 G8.1 锁带、medium M0.7875） | ☐ | B4 | design_track_b.md §7 |
| **B7** | M6 3D gate（旧 B4.5:TE 折线直纹面/g₂ 展向留空/尖端 Γ→0） | ☐ | B6 | design_track_b.md §7 |
| **B8** | 多尾流验证（多段翼/翼身组合体;旧 B5） | ☐ | B7 | DN1 §8 Phase B5 |
| **B9** | 曲线尾流 / 自由尾迹（旧 B6） | ⏸ 搁置 | — | DN1 §8 Phase B6; 决策见 DN2 §4.5.6 |

**B9 搁置原因**（DN2 §4.5.6）：精度收益微小 (O(0.1%))、工程代价不成比例、与 Newton 紧耦合结构性冲突、López 先例、优先级排序。保留 `update_direction()` 接口能力以备未来。

**Track B 与 Track P 的关系**：B1–B6 完成后可以替代当前 conforming 尾流路径；Track B 不阻塞 Track P 的 P7–P12（当前 conforming 尾流继续工作）。

**时序护栏（2026-07-10 记录;P8 已于 2026-07-11 关闭,前半兑现）**：P8 全耦合 Newton 的 Γ-Jacobian 块建立在 master-slave 消元上（`wake.py::self._h`），而 B3 隐式 Kutta 取消 Γ 自由度——两套 Newton 设计不并行推进。P8 已先在 conforming 尾流上落地；level-set 路径的 Newton 是 post-B6 的重推导（更简:尾迹 LS 块在 φ 中线性/常数,无 Γ 消元,见 design_track_b.md §5.5）。
...
| 8 | 直尾流 + 质量穿透，不做几何松弛 | 2026-07-10 | DN2 §4.5.6 | 5 条理由；B9 搁置 |
