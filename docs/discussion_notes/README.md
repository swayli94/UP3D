...
| # | 文件名 | 日期 | 主题 | 状态 | 备注 |
|---|--------|------|------|------|------|
| — | `PLAN.md` | 2026-07-10 | **统一开发计划** | ✅ 有效 | 整合 roadmap 进度 + design notes 理论决策；三条开发线 (P/B/V) 交叉引用；进度总览 |
| 1 | `20260707_1505_levelset_wake_design.md` | 2026-07-07 15:05（更新 07-10 16:15） | Level-Set 尾流 + 多值有限元实现方案 | ✅ 有效 | 方案 B 选定；6 阶段 B1-B6；已更新 P4 修复后的接口和性能基线；**B6（曲线尾流/自由尾迹）搁置**，交叉引用文档 2 §4.5.6 |
| 2 | `20260707_2118_ibl_viscous_coupling_design.md` | 2026-07-07 21:18（更新 07-10 16:00） | IBL 粘性修正耦合方案（VII） | ✅ 有效 | **Drela IBL3 6 方程**选定（替代 Green's 3-eq）；松→紧耦合渐进路线 V1-V4；V1 只依赖 P6，V3 依赖 P8；表面 Galerkin FE 避开流线追踪；§4.5 新增 TE 拐折+尾流偏折分析（局部基自适应/直尾流+质量穿透/不做几何松弛） |
...
| 5 | `20260708_1744_implementation_audit.md` | 2026-07-08 17:44 | UP3D 实现路径全面分析报告 | ✅ 有效 | 基于 López 博士论文 + Kratos 框架的交叉验证；理论/数值格式/实现逐层分析；P6/P7 技术清单；13 项风险 |
| 6 | `20260709_0145_3d_vii_implementation_analysis.md` | 2026-07-09 01:45 | 三维无粘-有粘迭代（VII）实现路径分析 | ✅ 有效 | 基于 López+Nishida-Drela 1995+Drela 2014；松→紧耦合渐进路线；壁面 transpiration BC+尾流面修正+IBL 求解器设计；14 项风险 |
...
| §4.5 TE 拐折+尾流偏折分析 | ✅ 新增 | Drela 局部基自适应吸收几何折转；直尾流+质量穿透松弛（López 路线）；不做尾流几何松弛（5 条理由：精度 O(0.1%)、工程代价、Newton 离散不连续性、López 先例、优先级） |
...
| 2026-07-08 17:44 | 新增文档 5 | UP3D 实现路径全面分析报告：基于 López 博士论文 + Kratos 框架的交叉验证；理论/数值格式/实现逐层分析；P6/P7 技术清单；13 项风险 |
| 2026-07-09 01:45 | 新增文档 6 | 三维 VII 实现路径分析：基于 López+Nishida-Drela 1995+Drela 2014；松→紧耦合渐进路线；壁面 transpiration BC+尾流面修正+IBL 求解器设计；14 项风险 |
| 2026-07-10 16:00 | 更新文档 2 | §4.5 新增 TE 拐折+有攻角尾流偏折分析：Drela 局部基自适应（§4.5.1-4.5.3）、尾流几何偏折与误差量级（§4.5.4-4.5.5）、直尾流+质量穿透松弛不做几何松弛的决策（§4.5.6，5 条理由） |
| 2026-07-10 16:15 | 更新文档 1 | Phase B6（曲线尾流/自由尾迹）从"可选"改为"搁置"，交叉引用文档 2 §4.5.6 |
| 2026-07-10 16:25 | 新增 PLAN.md | 统一开发计划：整合 roadmap 进度（P0-P10）+ design notes 理论决策（Track B/V）；三条开发线交叉引用；进度总览；关键设计决策记录 |
| 2026-07-10 | Track B/V 并入权威文档 + PLAN.md 修正 | roadmap.md 新增 Track B/Track V 章节 + ledger 行 + M2 备注（roadmap 为阶段状态唯一权威）；design.md §4（level-set 尾流指针 + B6 搁置）/§5（transpiration BC 指针）/§11（Track B/V 条目）；PLAN.md 修正 4 处：交叉引用路径、V3 验证方向（VII 使 CL **下降**向实验 0.26–0.27，0.245→0.288 是 P9 离散差距）、"P8 依赖 B3"→可选接口 + B3/P8 时序护栏、测试基线 157+4+2=163 |
