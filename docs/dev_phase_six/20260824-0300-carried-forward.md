...
（`docs/dev_phase_two/README.md` 那份 2026-08-10 检查单挡住了它们，理由有两条是**可测的事实**）。
...
★★ **这张表是 phase 6 的责任面**：GS6.1 的对象之一就是**把它变成一条会红的门**
（现在没有任何东西检查这些默认值有没有被静默改掉）。
...
★★ 而 §1 是唯一**实测**的一节，所以它也是唯一可以直接立门的一节 —— **GS6.1 从它开始。**
