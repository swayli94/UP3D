...
> ★ **事后标注(不改写):** 本文件 §1 把塌陷机制写成"每个限幅单元贡献 0.328,沿链**连乘**"。
> 那一句在细节上**错了** —— 一串限幅单元只算一道激波、只贡献一个因子;
> 实际的两条路径(无环长链 vs **donor 环**里的指针加倍平方)已在
> `20260731-2200-entropy-mcap-fix.md` §4b 实测并锁定。
> 结论不变(修法与判据都不依赖那句话),此处保留原文以存实录。
