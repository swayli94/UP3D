...
工作面——**B9 已关闭（2026-07-17，重定规格，用户批准）：翼身组合体跨模型验证，
LS（Picard）+ conforming（全新能力，Newton），M0.5/α3.06°，M2 coarse+medium；
两种尾迹模型在中网格上升力一致到 cl_p 0.4% / cl_kj 0.6%（P14 翼身版），GB9.4
机身升力 XFAIL⇒G1.6 机身 Cp 误差，LS Newton 在翼身上发散=neumann 远场阻塞而非
求解器**。粘性耦合线（V）设计完毕、零实现。
...
| **B — level-set 尾迹**（[roadmap/track_b.md](roadmap/track_b.md)） | B1–B5、B7、B8（characterized-not-cured）、B9、B11–B14、B15 ✓；B6 ◐（coarse gate ✓；medium 定量项由 GB15.4 补上） | **B9 ✓ 关闭 2026-07-17（重定规格，用户批准）**：翼身组合体跨模型验证 —— LS（Picard）+ conforming（全新能力，Newton）在中网格 M0.5 升力一致到 **cl_p 0.4% / cl_kj 0.6%**（P14 翼身版；coarse 12.8%=分辨率）。conforming 翼身新能力（`embed_wake=True`：机身两次 π-revolve 分片 + 尾迹片贯穿 + Netgen off；cut_wake/约束/P14 零改动；embed_wake=False 逐位不变）。GB9.4 机身升力 XFAIL（16-20%、随分辨率增长⇒G1.6 机身 Cp 误差，带未事后移动）；★ LS 用 Picard 而非 Newton——committed LS Newton 配方（Schur/freeze/lagged/ramp）在翼身上全发散，根因是 neumann 远场被机身阻塞（诊断：te_aux 完美 1.8e-8，8 个远场流体行 \|R\|≈84 在从未跑过的 freestream Newton 路径）。**B14 ✓ 2026-07-17**（`precond="schur"`）；B10 搁置 |
