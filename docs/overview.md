...
| Track | 状态 | 开放项 / 下一步 |
|-------|------|----------------|
| **P — 求解器**（[roadmap/track_p.md](roadmap/track_p.md)） | P0–P9 ✓（P1 仅 G1.6 以 strict xfail 挂起）；P10 ◐（G10.2/G10.3 ✓）；P13 ◐（G13.1 ✓、G13.2 conforming ✓、G13.3 亚声速 Richardson ✓ p=2.31） | G10.1（非升力 Newton 入口，无顺序约束）；G13.3 **跨声速阴性开放**（圆帽 fine 的 ramp 死于 M=0.75，site=尖 tip TE）；P11 条件性未开（仅剩 G1.6 理由）；P12 backlog；**P14 ◐ 2026-07-17 开启并落地（用户指示）**：壁面邻接 CV 压力相等 Kutta 估计器（A2 路由的修复）。**G14.1–G14.6 ✓**——S1/S2 一次换掉：M0.84 Γ(z) roughness 0.0970→**0.0043**（coarse）/ 0.0390→**0.0024**（medium，均达/优于 LS 带），raw TE Cp gap 0.318→**0.0040** / 0.228→**0.0024**（80×/95×，走 G14.6 **主条款**，回退条款未动用）。**★ G14.7 XFAIL 且不改带**：medium M0.84 cl_KJ 0.2823（+4.85% vs G8.2 **探针路径**锁）——机理在一级已测并在二级跑前**预注册**（两闭合逐点只差探针自身 O(h) 读数偏差，cross-read medium 0.79%；Kutta 映射 b≈0.93 把它 1/(1−b)≈14× 放大进 Γ）；**方向（记录，非 gate）**：\|cl_KJ−0.288\| 0.0188→0.0057，**P9 那 0.019 gap 关掉 69%**（P9 看不见它：两套网格共用同一估计器，偏差对其 Richardson 是共模）。**非**网格收敛断言（M6 fine 不是离散解）、**非**"0.019 是分辨率"翻案（仍 *strongly indicated, NOT earned*）、**非**"压力升力就是对的"。**待用户裁决**：认作发现（G14.7 改锚压力路径锁）还是当缺陷追。诚实记录：判别器 D=7.33→**1.80**（落在 A2 的 inconclusive 区，非 O(1)）；两路共有的 P1 恢复 spike（~0.08–0.1）**未治**（A2 分解的另一半仍开放）|
