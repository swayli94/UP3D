...
| Track | 状态 | 开放项 / 下一步 |
|-------|------|----------------|
| **P — 求解器**（[roadmap/track_p.md](roadmap/track_p.md)） | P0–P9 ✓（P1 仅 G1.6 以 strict xfail 挂起）；P10 ◐（G10.2/G10.3 ✓）；P13 ◐（G13.1 ✓、G13.2 conforming ✓、G13.3 亚声速 Richardson ✓ p=2.31） | G10.1（非升力 Newton 入口，无顺序约束）；G13.3 **跨声速阴性开放**（圆帽 fine 的 ramp 死于 M=0.75，site=尖 tip TE）；P11 条件性未开（仅剩 G1.6 理由）；P12 backlog；**P14 ◐ 2026-07-17 开启（用户指示）**（无探针 conforming Kutta 估计器，源自 A2——治 Γ(z) 抖动 + TE Cp gap；不在 B9 关键路径；两级 gates：一级 subsonic M0.5 里程碑 G14.1–G14.4、二级 transonic M0.84 G14.5–G14.7；接线范围仲裁 = Newton + Laplace；Stage-D 诊断 GO 20/20，`cases/analysis/p14_te_pressure_diag/`） |
...
406 − 7 = 399 才是 M2 这次的账；**A1 落地后基线即为 406**，届时不必再重跑对账。
