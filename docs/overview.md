...
工作面——**B9 重定规格并开启（2026-07-17，用户批准）：翼身组合体跨模型验证，
LS + conforming 双路 Newton，M0.5/α3.06°，M2 coarse+medium；conforming 翼身
是全新能力（带嵌入尾迹的网格变体），gate GB9.1–9.6 已在任何求解前预注册**。
粘性耦合线（V）设计完毕、零实现。
...
| **B — level-set 尾迹**（[roadmap/track_b.md](roadmap/track_b.md)） | B1–B5、B7、B8（characterized-not-cured）、B11–B14、B15 ✓；B6 ◐（coarse gate ✓；medium 定量项由 GB15.4 补上） | **B9 ◐ 重定规格+开启 2026-07-17（用户批准）**：翼身组合体跨模型验证 —— LS + conforming（conforming 翼身为**全新能力**：带嵌入尾迹的 `onera_m6_wingbody_conforming/` 网格变体，水线复制走既有边界边规则、cut_wake/约束/P14 Kutta 零改动），Newton，M0.5/α3.06°，M2 coarse+medium，对比 Γ(z)/cl(z)、截面 Cp、收敛、A1 耗时分解；GB9.1–9.6 在任何求解前预注册（GB9.5 跨模型 <1% @medium）；多元素腿移出未排期。**B14 ✓ 2026-07-17**（`precond="schur"`：A1 precond 瓶颈消失，M6 medium M0.84 42.6%→2.6%、ramp 1.43×/亚声速 2.08×；★ 小规模反而更慢）；B10 搁置 |
