...
|-------|--------|-----------|-------|
| M0 | ☐ (current priority) | | Re-specified 2026-07-06 as single-layer quasi-2D testbed (one extruded cell layer, globally consistent prism→3-tet split, embedded wake; acceptance-linked to gate G2.5 spanwise-zero); promoted to current priority, ready to start. No Gmsh script yet. (`cases/meshes/sphere_shell/` remains a P1/G1.2 validation case, not the M0 deliverable.) |
