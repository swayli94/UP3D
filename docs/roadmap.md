...
- [ ] G5.1 = V5: M∞ = 0.84 α = 3.06°: λ-shock topology; section Cp at
      ...
      *Coarse (55.5k): PASS — upper shock present/monotone/forward-migrating
      ...
      M>2 in the wake/far-field region near the tip (not the wing surface), see
      the ledger row. 3-level CL convergence scoped coarse+medium (no fine
      transonic per the cost decision).*
...
**OPEN — next-step development (blocks the medium gate, 2026-07-08).** The
medium (350.7k) solve is unphysical *only* through a handful of boundary
cells — this is the wake-tip / far-field failure mode the Risk note above
predicted, **not** a wing-flow problem. Precise symptom (cached-solution
diagnostic, `cases/demo/p5_onera_m6/results/medium_solution.npz`):
- **26 / 350 718 cells (0.007%) have M > 2**, M_max 5.20 → 8 floored / 4
  limited cells → the `physical` gate fails; V6 consistency 1.73% (> 1%).
- All spikes sit in the **chord-plane wake sheet near/behind the tip**:
  x ≈ 10 (adjacent to the R = 15 MAC far-field sphere), z/b ≳ 1.0, |y| ≈ 0
  (median 0.003), plus a few outboard-TE cells at x ≈ 1.08, z/b ≈ 0.80. The
  wing-surface section Cp is clean and its shocks are physical.
- Coarse (55.5k) has none of this (M_max 1.47, 0 floored) — the coarser wake/
  far-field cells don't develop the spike; the M0.75 continuation overshoot
  (M_max 11.4) that heals to 2.0 on coarse does **not** heal on the finer cells.

Leading hypothesis: the far-field Dirichlet uses a **2D point-vortex
correction** (`constraints/dirichlet.py`, design.md §5) that is a poor model
for the 3D swept wing exactly where the **chord-plane wake sheet meets the
far-field sphere near the tip** — producing a spurious potential gradient →
high local velocity there. Investigation plan:
1. Visualize φ / |∇φ| on the medium solution near x ≈ 10, z/b ≳ 1.0 to
   confirm the wake-sheet↔sphere corner is the source (VTU export exists).
2. Inspect the far-field Dirichlet magnitude/gradient the 2D-vortex term
   imposes on those tip-region far-field/wake nodes.
3. Test fixes in order of cost: (a) **taper the 2D-vortex correction to zero
   toward the tip / near the wake–far-field intersection**; (b) increase the
   far-field radius or refine the transition; (c) a proper 3D far-field
   (horseshoe/lifting-line) correction. Re-measure medium M_max, floored/
   limited count, V6 → targets: physical (0 floored/limited) and V6 < 1%.
Files: `constraints/dirichlet.py` (far-field vortex — the prime suspect),
`mesh/wake_cut.py` + `meshgen/wing3d.py` (wake sheet → sphere near the tip).
Note: a heavier iteration budget alone likely will **not** heal a boundary
artifact — fix the treatment first, then re-run the medium gate. This may be
scoped as a P5 sub-task or split off; it is orthogonal to P6 (surface-Cp
sawtooth) and P7 (Newton/speed).
