...
**Gates (Newton prerequisite = the frozen-selection differentiable walk flux):**
- [ ] G7.1 subcritical no-op: with any `max_ε` applied to the **inner** max only
      (outer clamp hard), the walk flux stays **bit-identical** to the P3 path at
      M∞→0 / ν≡0 (the G4.2 analogue).
- [ ] G7.2 solution preserved: optional `max_ε` reproduces the shipped walk's
      transonic solutions — G4.1 coarse+medium shock in the 0.62±0.03 band, G4.3
      10-case sweep converges, 0 floored/limited — i.e. the smoothing is
      negligible on the solution (no `upwind_c` recalibration; the walk is kept).
- [ ] G7.3 differentiability (core gate): finite-difference-verify ∂ρ̃/∂φ for
      the walk flux **at frozen selection** (rel err < 1e-6) in
      subsonic/accelerating/decelerating regimes — the exact property the P8
      Term 2/Term 3 Jacobian relies on (López B.3–B.6).
**Demo:** `cases/demo/p7_diff_flux/` (future) — frozen-walk ∂ρ̃/∂φ FD check +
`max_ε` solution-invariance; (optional) kernel-vs-walk speed/shock ladder.
**Effort:** 1 session (∂ρ̃/∂φ derivation + FD verification on the shipped walk;
`max_ε` optional). The kernel is already built — its calibration is deferred
optional work, not on the Newton path.
**Risk:** low — must preserve the subcritical no-op and the (M²−1)/M²
dissipation floor.
...
| P10 | ☐ | | Backlog (was P8): discrete adjoint (transpose of the P8 Newton Jacobian), VII transpiration BC (now expanded into Track V), mixed prism/tet + (C, M_c, ω) BO calibration. |
