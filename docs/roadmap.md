...
| M3 | **Wake-free ("O-mesh style") NACA quasi-2D family (NEW 2026-07-11, user-directed, for Track B).** Same single-layer extrusion pipeline as M0 but **without** the embedded wake surface — the analogue of an O-grid where M0's embedded sheet is the analogue of a C-grid's wake line. A wake-corridor refinement size field (wedge covering the intended α-sweep range; design_track_b.md D8) keeps element size along the expected wake comparable to M0's sheet-adjacent sizing, since without the sheet there is no natural refinement there. Coarse/medium family, one script, `.msh` gitignored like M0/M1. Wake-free M6 variant deferred until B4.5 needs it. | Solver preprocessor ingests it (no `wake` tag); quality report within M0 bounds; corridor sizing verified by an edge-length histogram along the wake wedge vs M0's sheet-adjacent sizing; B1 consumes it (Track B dual-mesh rule) |
...
|----|-------------|----------------------------------|
| B1 | Level-set wake + cut-element identification (`wake/levelset.py`, `wake/cut_elements.py`) | cut-element census, ext-DOF count, and ± side classification consistent on **both mesh types** (dual-mesh rule below): the wake-embedded M0 mesh (every wake-plane node exercises the ε side-shift) AND the wake-free M3 mesh (generic cuts) |
