...
- [ ] G6.1 surface-Cp smoothness: define an oscillation metric on the
      ...
      *(N1 progress 2026-07-08: metric built — `post/section_cut.py::
      cp_oscillation_metric` + `tests/test_p6_cp_metric.py`; coarse
      old-walk baseline measured = **0.0900** upper (n_super 58). **Metric
      refinement TODO:** the raw second difference also counts the shock
      foot's genuine steep gradient, so it must exclude the shock
      transition to isolate the 2-cell sawtooth — the kernel's aft/sharper
      shock inflated the raw number. Medium baseline not yet locked, see
      P6 ledger row.)*
...
- **Full Jacobian ("strategy A"), not an approximate one.** López keeps the
  ...
  coupling (B.4) → strict quadratic convergence (Tables 4.5/4.6/4.9). The
  upstream coupling widens the stencil **one element-layer**; recolor and
  extend the CSR scatter map accordingly. Dropping Term 3 → only superlinear.
