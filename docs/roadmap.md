...
Besides Track M (meshing) and Track P (solver), two further tracks are **designed
but not started**: **Track B** (level-set embedded wake) and **Track V**
(viscous–inviscid coupling) — see their sections after Track P. Their design
records live in `discussion_notes/` (DN1/DN2/DN6);
...
|----|-------------|----------------------------------|
| B1 | Level-set wake + cut-element identification (`wake/levelset.py`, `wake/cut_elements.py`) | **CLOSED 2026-07-11** (`tests/test_b1_cut_elements.py`, 25 passed, coarse+medium of BOTH families): (a) M0 embedded — every conforming sheet node ε-shifted "+" (D4 stress test at scale) and the census cross-validated **exactly** against the shipped preprocessor: `cut_elems ∪ te_lower_elems == cut_wake`'s minus-side element star, element-by-element; TE nodes == `wc.te_nodes`; (b) M3 wake-free — generic cuts form a gap-free corridor TE→far field at α=0 AND after `update_direction` to α=4° **on the same mesh**; per-node ext-DOF count == unique cut nodes. Delivered: TE-**polyline** ruled level set (D9, per-segment frames, synthetic 2-segment test), downstream-crossing test (excludes the ahead-of-LE sign-change region), `te_lower_elems` recorded for B2's López-fig-3.6c aux assignment |
...
- **Dual-mesh testing (NEW 2026-07-11, user-directed;
  ...
  wake-free M3 meshes (the "O-grid" analogue: generic cuts through generic
  elements — the actual workflow target). Where no conforming counterpart
  exists (M3), acceptance compares against the (a) results and external
  references.
