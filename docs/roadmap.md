...
> **Track-P renumber 2026-07-11 (user-directed):** this evidence phase is
> inserted as P9; the former P9 (curved walls) becomes **P10** and the former
> P10 (backlog) becomes **P11**. Docs written before 2026-07-11 use the old
> IDs (as with the 2026-07-08 renumber).

**Motivation.** Before committing to the large curved-walls effort (now P10),
...
If the gap Richardson-extrapolates away, P10's lift case collapses to
"improves the constant"; the sphere-Cp (G1.6) case for P10 stands regardless.
...
- [ ] G9.1 **M6 three-point grid study** at M0.84/α3.06: coarse/medium/fine
      ...
      ≤ 0.278 ⇒ saturating floor confirmed (P10 confirmed for lift);
...
### P10 — Curved / isoparametric wall elements ★ accuracy phase (renumbered from P9 on 2026-07-11; opening CONDITIONAL on the G9.3 verdict)
...
so the wall integral/gradient sees the true surface. **Gates:** G10.1 G1.6
sphere-Cp < 2% (medium); G10.2 V6 < 1% on the M6 medium gate. **Demo:**
`cases/demo/p10_curved_walls/` (future). **Effort:** large (own effort per DP1).
**Risk:** medium-high — isoparametric assembly is a real extension.

### P11 — Backlog (renumbered from P10 on 2026-07-11; post-v1.0, ordered by value to the MDO thread)
...
4. Non-lifting Newton entry point (sphere-class bodies currently run Picard
   only — capability boundary recorded by the P8 capability demo).
...
| P9 | ☐ (OPENED 2026-07-11) | | **Grid-convergence & accuracy-gap discrimination — evidence phase, NEW 2026-07-11 (user-directed renumber: former P9 curved walls → P10, former P10 backlog → P11).** Purpose: test the 0.019 M6 lift-gap attribution (cl_KJ 0.2692 vs Tranair/KRATOS 0.288 → "sharp-TE/LE P1 wall floor") BEFORE the large P10 effort — the P8 capability demo showed it is inference-grade (2D sharp-TE lift converges cleanly to the panel bracket; M6 cl_KJ still rising 0.2621→0.2692 under refinement, no sphere-style saturation). Gates with PRE-REGISTERED bands: G9.1 M6 coarse/medium/fine Newton + Richardson (cl_KJ∞ ≥ 0.283 ⇒ resolution-dominated; ≤ 0.278 ⇒ floor confirmed; else inconclusive); G9.2 NACA 2.5D coarse/medium/fine subsonic lift oracle (monotone error decrease + fine within ±1% of the PG–KT midpoint); G9.3 attribution verdict + P10 go/no-go recorded here (user arbitrates). No solver changes; fine meshes gitignored; heavy runs one-shot (CI budget untouched). |
| P10 | ☐ | | **Curved / isoparametric wall elements (renumbered from P9 on 2026-07-11; opening CONDITIONAL on G9.3):** the shared accuracy route for **G10.1** G1.6 sphere-Cp < 2% (design.md §5.1 Option C / DP1 "> 5%" branch) and **G10.2** the residual V6 < 1% floor left after P6 removes the sawtooth (attributed to the sharp-TE/LE P1 wall gradient — P9 tests this; M6 cl_KJ 0.2692 vs Tranair/KRATOS 0.288). Large own effort per DP1. |
| P11 | ☐ | | Backlog (renumbered from P10 on 2026-07-11; originally P8): discrete adjoint (transpose of the P8 Newton Jacobian), VII transpiration BC (now expanded into Track V), mixed prism/tet + (C, M_c, ω) BO calibration, non-lifting Newton entry point (P8 capability-demo boundary). |
