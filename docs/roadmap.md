...
and wake-free (M3, for Track B) tet meshes, coarse/medium/fine families per
case.
...
module docstring). M3 + M4 closed 2026-07-11 (gate rows above + ledger rows have
the delivery record): the M0/M1 pipelines with `embed_wake=False` — the corridor
refinement is size-field-only, so nothing in the mesh topology knows the wake
exists, which is exactly the form Track B's B1 consumes (dual-mesh rule; M3 =
quasi-2D, M4 = ONERA M6 so the swept TE and the wing tip are exercised too).
...
- [ ] G9.1 **M6 three-point grid study** at M0.84/α3.06: coarse/medium/fine
...
- [ ] G9.2 **2.5D sharp-TE lift oracle**: NACA0012 fine mesh (generated on
...
- [ ] G9.3 **attribution verdict recorded**: split the 0.019 gap into
...
`discussion_notes/PLAN.md`. **Status: B1 closed + M3 delivered 2026-07-11**
(`pyfp3d/wake/{levelset,cut_elements}.py`, dual-mesh gate green on coarse+medium
of both families); next = B2 (multivalued FE assembly, consuming B1's
`dofs_upper`/`dofs_lower` tables + `te_lower_elems`).
...
> (DN6 internally also uses "V2 = 3D crossflow" in one diagram — the phase table
> below, mirrored in PLAN.md, is the binding one.)

| ID | Deliverable | Gate sketch | Prereq |
|----|-------------|-------------|--------|
| V1 | IBL3 solver + loose coupling: `viscous/ibl3.py`, `viscous/transpiration.py` (δ*→ṁ), `viscous/coupling.py`; wall-Neumann blowing source in `kernels/residual.py`; wake-sheet mass source in `constraints/wake.py` | GV1.1 2D NACA0012 subsonic δ* vs XFOIL; GV1.2 loose loop converges in 5–10 iterations; GV1.3 δ* = 0 bit-identical to the inviscid path | P6 ✓ (smoothed wall gradient is the IBL edge-velocity input) |
| V2 | Quasi-simultaneous coupling (Hilbert-integral potential surrogate, `viscous/hilbert.py`) — **optional**, skip if V1 converges fast enough | GV2.1 30–50% fewer coupling iterations; improved robustness near separation | V1 |
| V3 | Tight coupling: augmented (φ, Γ, δ, A, B, Ψ, C_τ1, C_τ2) Newton; J_φ,δ* = ∂ṁ/∂δ* and J_δ*,φ blocks; GMRES + block preconditioning (AMG potential / ILU BL) | GV3.1 quadratic convergence; GV3.2 2D transonic VII shock shift vs experiment; GV3.3 M6: CL moves **down** from the converged inviscid value toward experiment ≈ 0.26–0.27 (direction + magnitude, DN6 §13.3) | P8 + V1 |
| V4 | Wake-sheet IBL correction — a continuation of V1, **not** an independent phase (same 6 equations, wake closure relations; reserve the wake unknowns in V1's data layout) | δ*_wake enters as the wake-sheet RHS mass source; TE kink absorbed by Drela local-basis adaptation (DN2 §4.5.1–4.5.3); straight wake + mass-transpiration relaxation, no geometric relaxation (DN2 §4.5.6) | V1 |
...
- **VII does not close the inviscid-discretization CL gap.** The M6 CL 0.245 vs
  the FP-reference 0.288 is the sharp-TE/LE P1 wall-gradient floor → **P9**
  (curved elements). Viscosity moves CL *down* from the (accurate) inviscid
  value toward experiment — do not book the 0.245→0.288 recovery to Track V.
