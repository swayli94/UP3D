...
Corrected root cause: **a trailing-edge / Kutta discretization singularity
fed by the per-station spanwise-Γ noise at the outboard roll-off** (dominant,
18 cells), plus a **secondary 2D-point-vortex far-field artifact at the tip**
...
sharpening + single-station localization rule that out — though note the npz
caches only aggregate scalars, so the demo should also persist
`residual_history`/`gamma_history` to make the convergence curve directly
inspectable next time). Investigation / fix plan, by leverage:
1. **Regularize the spanwise Γ distribution** — spanwise
   smoothing/relaxation of the per-station Kutta Γ (or a smoothness
   constraint across stations) so dΓ/dz at z/b ≈ 0.80 stops injecting a
   velocity spike into the TE tets. Targets the 18-cell dominant cluster.
   Files: `constraints/wake.py`, `solve/picard.py` (per-station Kutta loop).
2. **Taper the 2D-vortex far-field correction toward the tip**
   (`constraints/dirichlet.py`) — removes the single M_max = 5.20 far-field
   cell; necessary but NOT sufficient for the `physical` gate on its own.
3. Re-measure medium M_max, floored/limited count, V6 → targets: physical
   (0 floored/limited) and V6 < 1%.
Note: this is orthogonal to P6 (surface-Cp sawtooth) and P7 (Newton/speed),
but the TE-singularity mechanism overlaps the wake/TE modeling those phases
touch. Fix the Γ-smoothness treatment first, then re-run the medium gate.
