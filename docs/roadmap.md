...
- [x] **G13.2 — tip-edge desingularization (CONFORMING path CLOSED 2026-07-13;
      ...
      **M_max 2.818, 0 cells over M_cap, 0 limited, 0 floored, converged**
      (38 min). Coarse/medium likewise 0-limited, and the medium `M_max` drops
...
- [ ] **G13.3 — 3D grid-convergence closure (PARTIALLY ANSWERED; formal
      ...
      **GROW** (M0.84: +2.3 %, +8.1 %; M0.5: +0.2 %, +5.8 %) ⇒ **no extrapolation
      was run** (P9/G9.3 discipline: report `n/a`, never fabricate); (b) the M6
      **mesh family is not self-similar** — `h_far = min(2.5, 120·h_wall)` clamps
      at coarse, so coarse→medium refines the far field 1.39× against the wall's
      2× ⇒ **`coarse` is not on the refinement ray and can never legally enter a
      3-point Richardson** (this also explains the coarse→medium spanwise
