...
[phases/p1/docs/roadmap/track_a.md](../../phases/p1/docs/roadmap/track_a.md); conclusions:
[phases/p1/docs/demo_report/track_a.md](../../phases/p1/docs/demo_report/track_a.md).

- `a1_solver_bottleneck/` — A1 conforming-vs-level-set × Picard-vs-Newton cost benchmark (GA1.1–GA1.5) —
  `python bench/studies/a1_solver_bottleneck/run_a1.py` — ~5 min (+ gated 3-D `run_a1_m6.py` ~1 h)
- `a2_te_kutta_fidelity/` — A2 TE/Kutta fidelity attribution (GA2.1–GA2.5; S1 jitter + S2 TE Cp gap) —
  `python bench/studies/a2_te_kutta_fidelity/run_a2.py` — ~90 s (+ gated `run_a2_interventions.py`)
- `a4_ue_error_band/` — A4 wall edge-velocity (u_e) error-band study = the Track-V IBL input-quality
  ...
  `python phases/p1/cases/analysis/a4_ue_error_band/run.py` — ~30 s (cheap M0 solves, no cache)
- `c1_ls_jacobian_fd/` — **A3 / GA3.5 + B19 + B20 + B21** — is the LS Newton Jacobian exact on a 3D mesh?
  ...
  `results/n1_freeze_fix_sweep.csv`) — `python phases/p1/cases/analysis/c1_ls_jacobian_fd/run_check.py` ·
...
- `p14_te_pressure_diag/` — P14 pressure-Kutta diagnostics (20 checks; feeds the P14 demo) —
  `python bench/studies/p14_te_pressure_diag/run_diag.py` — heavy
- `b9_fuselage_guardrail/` — B9 fuselage-Cp guardrail (GB9.6, RECORDED — no pass/fail) —
  `python phases/p1/cases/analysis/b9_fuselage_guardrail/run_guardrail.py` — heavy
- `b23_junction_discriminator/` — B23 wing-body junction discriminator (P11 close-out mandate:
  ...
  `python phases/p1/cases/analysis/b23_junction_discriminator/run_d1.py` · `.../run_d2.py` · `.../run_d4.py` ·
...
- `b24_wake_inboard_end/` — B24 wake inboard free-end fix A/B (executes the B23 verdict:
  ...
  `python phases/p1/cases/analysis/b24_wake_inboard_end/run_e1.py` — heavy (reuses the b23 meshes + control data)
- `b25_inboard_fragment_clip/` — B25 wake inboard fragment clip (executes B23 §(b)-2 as the conforming-topology fix:
  ...
  `python phases/p1/cases/analysis/b25_inboard_fragment_clip/run_f1.py` — medium (reuses the b23 meshes + control data)
- `b26_ls_transonic_ceiling/` — B26 pocket-healed LS transonic ceiling re-measure (same-code A/C with the only
  ...
  `python bench/studies/b26_ls_transonic_ceiling/run_g1.py` — heavy (4 ramps, ~69 min solve)
...
- `b28_cl_fus_flat_sheet/` — B28 cl_fus out-band flat-vs-tilted decoupling + GB9.4 re-spec (decisive leg: flat-fragment
  ...
  `python bench/studies/b28_cl_fus_flat_sheet/run_f2.py` — medium (~40 min)
- `b32_tip_taper_adoption/` — B32 ② weld-sign freeze fix + ① conforming taper production adoption (user-adjudicated from
  ...
  `python phases/p1/cases/analysis/b32_tip_taper_adoption/run_g1.py` · `.../run_g2.py` — solves cached, CSV/PNG regenerate in minutes
- `b31_tip_termination/` — B31 C-class wing-tip cure (sheet-termination re-spec) + LS step-semantics companion
  ...
  bit-identical) — `python phases/p1/cases/analysis/b31_tip_termination/run_g1.py` · `.../run_g2.py` · `.../run_g2b.py` ·
...
- `b30_transonic_ceiling/` — B30 (b)-class wing-body transonic ceiling attribution + López dissipation lever
  ...
  `python phases/p1/cases/analysis/b30_transonic_ceiling/run_g1.py` · `.../run_g2.py` · `.../run_g3.py` — all solves cached,
...
- `v1_ibl3_standalone/` — **Track V / V1** GV1.1 standalone IBL3 verification (prescribed u_e, no FP coupling:
  ...
  `python bench/studies/v1_ibl3_standalone/run.py` — ~7 min (exit 1 = honest FAIL present)
- `v2_transpiration_channel/` — **Track V / V2** GV2.1 transpiration channel verification (δ*→ṁ = ∇_Γ·(ρ_e u_e δ*)
  ...
  `python bench/studies/v2_transpiration_channel/run.py` — heavy (exit 1 = honest FAIL present)
- `v3_loose_coupling/` — **Track V / V3** GV3.1/GV3.2 loose viscous–inviscid coupling on NACA0012 2.5-D strip
  ...
  **2 PASS / 4 FAIL / 23 RECORDED** — PASS: Δcl ratio 0.542 ∈ [0.5, 2.0], GV3.2 loop convergence 5 outer iters ω=1.0
  ...
  `docs/design_track_v.md` §10) — `python bench/studies/v3_loose_coupling/run.py` — heavy (exit 1 = honest FAIL
  present)
- `v3_fuselage_smoke/` — **Track V / V3** GV3.3 fuselage body-of-revolution smoke (Track V's only fuselage-alone
  ...
  record `docs/design_track_v.md` §10) — `python phases/p1/cases/analysis/v3_fuselage_smoke/run.py --levels coarse` —
...
- `v5_1b_scaled_newton/` — **Track V / V5** GV5.1b scaled + damped augmented Newton (the GV5.1
  ...
  `python bench/studies/v5_1b_scaled_newton/run.py` — coarse leg ~5 min / medium ~27 min /
...
- `v5_1c_above_band_window/` — **Track V / V5** GV5.1c the above-band seed: the pre-floor
  ...
  **no quadratic regime anywhere above the floor** — the clean-descent steps are line-search-capped
  ...
  `python bench/studies/v5_1c_above_band_window/run.py` — coarse ~3 min / medium ~3 min at
...
- `v5_1d_near_band_window/` — **Track V / V5** GV5.1d the near-band seed: does a quadratic
  ...
  `python bench/studies/v5_1d_near_band_window/run.py` — coarse ~0.5 min / medium ~1.3 min
...
- `v5_5_te_floor/` — **Track V / V5** GV5.5 TE-band (B, δ) formulation — breaking the IBL
  ...
  **245998×** the seed's own flag-OFF floor (medium — the 8-thread scatter clause fired,
  ...
  `python bench/studies/v5_5_te_floor/run.py` — coarse ~6 min / medium ~12 min
...
- `v5_2_rae2822/` — **Track V / V5** GV5.2 RAE2822 transonic VII vs committed experiment
  ...
  `python bench/studies/v5_2_rae2822/run.py` — ~43 min at 8 threads unloaded
...
- `v5_3_m6_cp/` — **Track V / V5** GV5.3 M6 wing direction+magnitude check vs
  ...
  `python bench/studies/v5_3_m6_cp/run.py` — coarse ~29 min / medium ~3.5 h
...
- `v5_4_cost/` — **Track V / V5** GV5.4 augmented-step cost on M6 medium
  ...
  **band (a) RECORDED**: augmented step 22.93 s vs the in-session inviscid
  ...
  `python bench/studies/v5_4_cost/run.py` — ~25 min total at 8 threads
...
- `v5_6_schur_prec/` — **Track V / V5** GV5.6 Schur-aware reduced-space
  ...
  **band (a) RECORDED** 23.88 s / 3.03 s = **7.87×** (above ≤ ~2×,
  ...
  `python phases/p1/cases/analysis/v5_6_schur_prec/run.py` — ~21 min total at 16
...
- `v5_ibl_floor/` — **Track V / V5** IBL-floor diagnosis (the GV5.1 follow-up; RECORDED diagnostic study, no
  ...
  `python bench/studies/v5_ibl_floor/run.py` — ~50 min under external load (2026-07-24 measurement;
...
- `v5_m6_bridge/` — **Track V / V5** GV5.0 M6 subsonic loose-coupling bridge (RECORDED entry check: ONERA M6
  ...
  **16 RECORDED / 0 FAIL** — bridge answer: the loose loop does NOT converge ≤10 at either level; coarse =
  ...
  `python bench/studies/v5_m6_bridge/run.py --levels coarse` — coarse ~25 min / medium ~6.5 h (exit 1 = honest
  FAIL present)
- `v5_tight_coupling/` — **Track V / V5** GV5.1 tight-coupling exactness + convergence (augmented (φ, Γ, BL)
  ...
  `python bench/studies/v5_tight_coupling/run.py --levels coarse` — coarse ~1 min / medium ~3 min
...
- `v6_1_wake_sheet/` — **Track V / V6** GV6.1 conforming wake-sheet δ* source (pre-registered
  ...
  CSVs/PNG in the dir) —
  `python bench/studies/v6_1_wake_sheet/run.py` — ~6 min at 8 threads (2026-07-25 measurement;
...
- `v6_2_measured_effect/` — **Track V / V6** GV6.2 measured wake-IBL on/off effect vs the A4
  band (pre-registered `ab89483` before execution; the XFOIL wake-reference sourcing RULED
  Option A (user) — analysis-local rows, `cases/reference_data/` untouched, G3 polar
  reproduction in-line; **0 PASS / 0 FAIL / 24 RECORDED** — the on/off Δ-cl +0.00015
  (+0.0547 %) / TE-region max |ΔCp| 0.00250 sits at 0.022×/0.051× the A4 input band = NOT
  significant, L-robust over L_rel ∈ {0.5, 1.0, 2.0} c (1.0 c stays the pinned MODEL
  ...
  `python bench/studies/v6_2_measured_effect/run.py` — ~5 min at 8 threads (2026-07-25
