...
| `b25_inboard_edge_taper/` | B25 inboard free-edge constraint-side treatment (executes B23 §(b)-2: B8 row-blend/span-blend welds rescoped from the tip anchor to the root anchor — pocket vs root-lift trade; pre-registered with the B8 dead-oracle declared in `PRE_REGISTRATION.md`) | `python cases/analysis/b25_inboard_edge_taper/run_t1.py` | medium (reuses the b23 meshes + control data) |
