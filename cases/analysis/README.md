...
| `b24_wake_inboard_end/` | B24 wake inboard free-end fix A/B (executes the B23 verdict: TE-polyline inboard end extended along the fuselage waterline so the near field has no free edge; criteria = pocket gone/moved + wing loads unchanged; pre-registered in `PRE_REGISTRATION.md`) | `python cases/analysis/b24_wake_inboard_end/run_e1.py` | heavy (reuses the b23 meshes + control data) |
