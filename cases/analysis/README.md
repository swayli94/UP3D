...
| `c1_ls_jacobian_fd/` | **A3 / GA3.5 + B19** — is the LS Newton Jacobian exact on a 3D mesh? (kimi code-review C1; `run_check.py` measures it before/after the B19 fix, `run_legb_probe.py` measures B19 Leg B's residual asymmetry; `run_legb_apply.py` / `run_legb_beforeafter.py` / `run_legb_b18.py` are B20's before/after on the `plain_density="main"` fix) | `python cases/analysis/c1_ls_jacobian_fd/run_check.py` · `.../run_legb_probe.py` | ~4 min each |
