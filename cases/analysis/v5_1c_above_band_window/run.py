"""V5.1c -- the above-band seed: a true read of the pre-floor slope-2 window.

Binding text: cases/analysis/v5_1c_above_band_window/PRE_REGISTRATION.md
(committed BEFORE the first execution, per discipline). Design inputs: the
committed GV5.1b VERDICT (../v5_1b_scaled_newton/VERDICT.md Sec.4-5 -- the
amended seeds sit at 1.00x the diagnosed floor, no above-band contraction
segment exists by construction) and the IBL-floor diagnosis
(../v5_ibl_floor/results/findings.md).

Protocol:
  base seed = the GV5.1 amended protocol verbatim (the GV5.1b runner's
  build_loose_state IMPORTED, not mirrored -- HEAD-regen loose-converged
  state, wiring guard |dcl_k0| <= 1e-8); the tight pack is built on the
  UNPERTURBED state (every frozen operator identical to GV5.1b's);
  perturbed seed = U[:,0] * (1+eps) at the FREE (non-inflow) BL nodes,
  eps calibrated by a deterministic log10-bisection into the
  pre-registered windows T1=[5e-2,5e-1] (primary) / T2=[5e-1,5e0]
  (escalation, fires ONLY if the T1 trajectory yields <3 above-band
  contraction triples; one re-seed max per level);
  solve = the GV5.1b machinery verbatim: newton_tight(scaling="rowcol",
  lm_damping=True, floor_stop=True; max_iter=10, tol=1e-8,
  tol_abs=1e-10, line_search=True). Assembly untouched; no solver-side
  edits of any kind.

Bands (the pre-registered ones):
  (a) implementation exactness: the committed suite green
      (tests/test_v5_tight_scaled.py 8 + the tight fleet 28 + the new
      synthetic seed-helper tests tests/test_v5_above_band_seed.py) +
      live identities on the PERTURBED-seed J: e1 <= 1e-12; e2 <=
      max(1e-10, 10*kappa_1(J)*eps) -- the cond-aware tolerance
      PRE-REGISTERED (the GV5.1b 2026-07-24 adjudication carried in);
      e3 <= 1e-6.
  (b) the pre-floor slope-2 window (medium binding, coarse recorded):
      p on F_BL max-norm contraction triples with both predecessors
      above the floor band (coarse 3.16e-5 / medium 1.71e-5, the GV5.1b
      bands verbatim); PASS = >= 3 triples (T1; pooled with T2 if the
      escalation fires) with median p in [1.5, 2.5]; converged outright
      -> trivially PASS; < 3 triples after the escalation -> the
      pre-registered RECORDED fallback (available triples + per-step
      contraction factors + the log-log regression slope over above-band
      pairs quoted; NOT a gate crash).
  (c) counts (recorded; aspirational N_polish <= 2x loose; the seeds
      are deliberately far -- N_polish measures the window traversal,
      not production convergence).

Environment: executed under a TEMPORARY 8-thread constraint (this
session only, user-directed 2026-07-24): the runner keeps the 16
defaults below, the constraint is applied via the environment at
execution time and recorded in the artifacts; wall times are NOT
comparable to the 16-thread ledger entries.

Run:  python cases/analysis/v5_1c_above_band_window/run.py
      python cases/analysis/v5_1c_above_band_window/run.py --levels coarse
(partial runs merge summary.csv by (band, level); a full run
regenerates every artifact from scratch)
"""

import argparse
import csv
import importlib.util
import os
import sys
import time

# resolve pyfp3d from THIS worktree (the site-packages editable install may
# point at a sibling worktree)
sys.path.insert(0, os.path.abspath(os.path.join(
    os.path.dirname(__file__), "..", "..", "..")))

# runner defaults (the ledger-standard 16); the temporary session
# constraint (8) is applied via the environment, never here
os.environ.setdefault("NUMBA_NUM_THREADS", "16")
os.environ.setdefault("OMP_NUM_THREADS", "16")
os.environ.setdefault("OPENBLAS_NUM_THREADS", "16")

import numpy as np
import scipy.sparse as sp
import scipy.sparse.linalg as sla

from pyfp3d.viscous import tight_driver as td
from tests.v5_state import M_CAP, M_CRIT, RHO_FLOOR, UPWIND_C

HERE = os.path.dirname(os.path.abspath(__file__))
RESULTS = os.path.join(HERE, "results")
os.makedirs(RESULTS, exist_ok=True)

GV51B_RUN = os.path.join(HERE, "..", "v5_1b_scaled_newton", "run.py")
GV51B_RESULTS = os.path.join(HERE, "..", "v5_1b_scaled_newton", "results")


def _load_gv51b():
    """The COMMITTED GV5.1b runner module -- imported, not mirrored, so
    the seed construction and the protocol reads cannot drift."""
    spec = importlib.util.spec_from_file_location("gv51b_run", GV51B_RUN)
    mod = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(mod)
    return mod


gb = _load_gv51b()

FLOOR_BAND = gb.FLOOR_BAND          # coarse 3.16e-5 / medium 1.71e-5
TARGETS = {"T1": (5.0e-2, 5.0e-1), "T2": (5.0e-1, 5.0e0)}
EPS_BRACKET = (1.0e-8, 1.0e4)
MAX_CALIB_EVALS = 20
BAND_A_TOL = gb.BAND_A_TOL          # (1e-12, 1e-10, 1e-6)
E2_COND_SAFETY = gb.E2_COND_SAFETY  # 10.0
THREADS = os.environ.get("NUMBA_NUM_THREADS", "16")

SUMMARY = []                        # (band, level, key_result, verdict)


# ---------------------------------------------------------------------------
# summary/compare writers (the v5_tight_coupling merge discipline, runner-local)
# ---------------------------------------------------------------------------


def _write_lines(name, header, lines):
    path = os.path.join(RESULTS, name)
    with open(path, "w") as f:
        f.write(header + "\n")
        for ln in lines:
            f.write(ln + "\n")
    print(f"  wrote {path}", flush=True)


def _write_summary():
    rows = {}
    path = os.path.join(RESULTS, "summary.csv")
    if os.path.exists(path):
        with open(path) as f:
            lines = [ln.rstrip("\n") for ln in f if ln.strip()]
        for ln in lines[1:]:
            flds = ln.split(",", 2)
            if len(flds) >= 2:
                rows[(flds[0], flds[1])] = ln
    for t in SUMMARY:
        rows[(t[0], t[1])] = ",".join(str(x) for x in t)
    order = {"(a)": 0, "(b)": 1, "(c)": 2, "calib": 3, "wall": 4}
    out = sorted(rows.values(),
                 key=lambda ln: (order.get(ln.split(",", 1)[0], 99),
                                 ln.split(",", 2)[1]))
    _write_lines("summary.csv", "band,level,key_result,verdict", out)


def _record(band, level, key_result, verdict):
    SUMMARY.append((band, level, key_result, verdict))
    print(f"  [{verdict}] {band} {level}: {key_result}", flush=True)
    _write_summary()


def _merge_compare(level, rows):
    """compare.csv: one row per (level, metric); fresh rows replace the
    same pair, other levels' rows kept."""
    header = "level,metric,gv5_1c,committed_reference,note"
    kept = {}
    path = os.path.join(RESULTS, "compare.csv")
    if os.path.exists(path):
        with open(path) as f:
            lines = [ln.rstrip("\n") for ln in f if ln.strip()]
        for ln in lines[1:]:
            flds = ln.split(",", 2)
            if len(flds) >= 2:
                kept[(flds[0], flds[1])] = ln
    for r in rows:
        kept[(level, str(r[0]))] = ",".join(str(x) for x in (level,) + r)
    order = {"coarse": 0, "medium": 1}
    out = sorted(kept.values(),
                 key=lambda ln: (order.get(ln.split(",", 1)[0], 9),
                                 ln.split(",", 2)[1]))
    _write_lines("compare.csv", header, out)


# ---------------------------------------------------------------------------
# the above-band seed: perturbation + calibration (pure helpers, suite-gated)
# ---------------------------------------------------------------------------


def perturb_delta(x, n_free, n_st, inflow_mask, eps):
    """x -> x with U[:, 0] (delta) scaled by (1+eps) at the FREE
    (non-inflow) BL nodes only; the inflow Dirichlet band is boundary
    data and stays untouched (the pre-registered perturbation)."""
    x2 = np.asarray(x, dtype=np.float64).copy()
    U = x2[n_free + n_st:].reshape(-1, 6)
    free = ~np.asarray(inflow_mask, dtype=bool)
    U[free, 0] *= (1.0 + eps)
    return x2


def calibrate_eps(fbl_fn, window, eps_lo=EPS_BRACKET[0],
                  eps_hi=EPS_BRACKET[1], max_evals=MAX_CALIB_EVALS):
    """Deterministic log10 bisection: find eps with fbl_fn(eps) inside
    window=(lo, hi). fbl_fn is inf-safe (inf on throw/non-finite steers
    the bracket down). Returns (eps, fbl, trace, ok, reason) with trace
    = list[(eval_idx, eps, fbl)]."""
    lo_w, hi_w = window
    trace = []

    def ev(eps):
        f = float(fbl_fn(eps))
        trace.append((len(trace), eps, f))
        return f

    f_lo, f_hi = ev(eps_lo), ev(eps_hi)
    if lo_w <= f_lo <= hi_w:
        return eps_lo, f_lo, trace, True, "bracket-lo in window"
    if lo_w <= f_hi <= hi_w:
        return eps_hi, f_hi, trace, True, "bracket-hi in window"
    if f_hi < lo_w:
        return None, f_hi, trace, False, (
            f"fbl(eps_hi={eps_hi:.0e})={f_hi:.3e} below the window "
            f"[{lo_w:.0e},{hi_w:.0e}] -- unreachable")
    if f_lo > hi_w:
        return None, f_lo, trace, False, (
            f"fbl(eps_lo={eps_lo:.0e})={f_lo:.3e} above the window "
            f"[{lo_w:.0e},{hi_w:.0e}] -- overshoot at the bracket floor")
    while len(trace) < max_evals:
        mid = float(np.sqrt(eps_lo * eps_hi))
        f = ev(mid)
        if lo_w <= f <= hi_w:
            return mid, f, trace, True, "bisection in window"
        if f < lo_w:
            eps_lo = mid
        else:  # f > hi_w or inf
            eps_hi = mid
    return None, trace[-1][2], trace, False, (
        f"bisection exhausted ({max_evals} evals) without landing in "
        f"[{lo_w:.0e},{hi_w:.0e}] -- non-monotone response recorded")


# ---------------------------------------------------------------------------
# the band-(b) reads (pure helpers, suite-gated)
# ---------------------------------------------------------------------------


def regression_slope(e, band):
    """Least-squares slope of log10(e_{i+1}) vs log10(e_i) over
    above-band consecutive contraction pairs (e_i > band, e_{i+1} <
    e_i). Returns (slope or None, n_pairs)."""
    xs, ys = [], []
    for i in range(len(e) - 1):
        if e[i] > band and 0.0 < e[i + 1] < e[i]:
            xs.append(float(np.log10(e[i])))
            ys.append(float(np.log10(e[i + 1])))
    if len(xs) < 3:
        return None, len(xs)
    slope = float(np.polyfit(np.array(xs), np.array(ys), 1)[0])
    return slope, len(xs)


def contraction_factors(e, band):
    """log10(e_{i-1}/e_i) on contraction steps whose predecessor sits
    above the band. Returns list[(iter, decades)]."""
    return [(i, float(np.log10(e[i - 1] / e[i])))
            for i in range(1, len(e))
            if e[i - 1] > band and 0.0 < e[i] < e[i - 1]]


def window_verdict(triples, converged, level):
    """The pre-registered band-(b) verdict on the (possibly pooled)
    above-band triples list [(iter, p)]. converged outright -> PASS;
    >= 3 triples -> median p in [1.5, 2.5] (medium binding; coarse
    RECORDED); < 3 -> RECORDED fallback."""
    if converged:
        return "PASS", "converged outright -- the band is trivially PASS"
    if len(triples) >= 3:
        med = float(np.median([p for _, p in triples]))
        hit = 1.5 <= med <= 2.5
        verdict = ("RECORDED" if level == "coarse"
                   else ("PASS" if hit else "FAIL"))
        return verdict, (f">= 3 above-band contraction triples; median "
                         f"p={med:.2f} vs [1.5..2.5]")
    return "RECORDED", ("< 3 above-band contraction triples after the "
                        "escalation -- the pre-registered fallback read")


# ---------------------------------------------------------------------------
# live reads on the pack
# ---------------------------------------------------------------------------


def fbl_of(pack, x):
    """F_BL max-norm at x (the history's block_max[2] read, td._block_max);
    inf on throw/non-finite -- the calibration guard (deterministic
    steering; NOT a change to any probe guard)."""
    try:
        F = td.augmented_residual(pack, x)
    except (ArithmeticError, SystemError):
        return float("inf")
    if not np.all(np.isfinite(F)):
        return float("inf")
    return float(td._block_max(pack, F)[2])


def band_a_live_at(pack, x0):
    """The GV5.1b band-(a) live identities, parameterized on the
    evaluation state (here: the perturbed T1 seed J): e1 = diag-scaling
    round-trip; e2 = mu=0 damped step vs the undamped splu step against
    the PRE-REGISTERED cond-aware tolerance max(1e-10, 10*kappa_1*eps);
    e3 = the dx = C dy round-trip. The machinery itself is gated by the
    committed suite (tests/test_v5_tight_scaled.py)."""
    F0 = td.augmented_residual(pack, x0)
    J0 = td.augmented_jacobian(pack, x0)
    rn, cn, Jsc = td.equilibrate_rc(J0)
    E = (sp.diags(rn) @ Jsc @ sp.diags(cn) - J0).tocsr()
    e1 = float(abs(E.data).max() / abs(J0.data).max())
    try:
        lu = sla.splu(J0.tocsc())
        d_ref = -lu.solve(F0)
        scale = max(float(np.max(np.abs(d_ref))), 1e-300)
        d0 = td.scaled_damped_step(J0, F0, 0.0)
        e2 = float(np.max(np.abs(d0 - d_ref)) / scale)
        d_rc = td.scaled_damped_step(J0, F0, 0.0, scaling="rowcol")
        e3 = float(np.max(np.abs(d_rc - d_ref)) / scale)
        cond1 = float(
            sla.onenormest(J0)
            * sla.onenormest(sla.LinearOperator(
                J0.shape, matvec=lu.solve,
                rmatvec=lambda b: lu.solve(b, trans="T"))))
        tol_e2 = max(BAND_A_TOL[1],
                     E2_COND_SAFETY * cond1 * np.finfo(float).eps)
    except RuntimeError as exc:
        print(f"    band (a) live: seed J splu-singular ({exc}) -- "
              "e2/e3 recorded nan", flush=True)
        e2 = e3 = tol_e2 = float("nan")
    ok = (e1 <= BAND_A_TOL[0]
          and (np.isnan(e2) or e2 <= tol_e2)
          and (np.isnan(e3) or e3 <= BAND_A_TOL[2]))
    print(f"    band (a) live on the perturbed T1-seed J: e1={e1:.3e} (<="
          f"{BAND_A_TOL[0]:.0e}) e2={e2:.3e} (<={tol_e2:.0e} cond-aware, "
          f"pre-registered) e3={e3:.3e} (<={BAND_A_TOL[2]:.0e}) -> "
          f"{'PASS' if ok else 'FAIL'}", flush=True)
    return e1, e2, e3, tol_e2, ok


def _gv51b_final(level):
    """The GV5.1b committed final row (f_bl_max, merit) -- read, never
    recomputed."""
    with open(os.path.join(
            GV51B_RESULTS, f"newton_history_{level}.csv")) as f:
        last = list(csv.DictReader(f))[-1]
    return float(last["f_bl_max"]), float(last["merit"])


def _write_history(tag, hist, band, term):
    """newton_history_{tag}.csv -- the GV5.1b columns verbatim."""
    e_bl = [float(h["block_max"][2]) for h in hist]
    m_seq = [float(h["merit"]) for h in hist]
    p_bl = dict(gb._p_series(e_bl, band=band))
    p_m = dict(gb._p_series(m_seq))
    lines = []
    for i, h in enumerate(hist):
        bm = h["block_max"]
        lines.append(",".join([
            str(h["iter"]),
            f"{bm[0]:.6e}", f"{bm[1]:.6e}", f"{bm[2]:.6e}",
            f"{h['merit']:.6e}",
            "" if h.get("mu") is None else f"{h['mu']:.6e}",
            "" if h["lam"] is None else f"{h['lam']:.6f}",
            "" if i not in p_bl else f"{p_bl[i]:.4f}",
            "" if i not in p_m else f"{p_m[i]:.4f}",
            f"{h['ds_change']:.6e}",
            term if i == len(hist) - 1 else "",
            f"{h['wall_s']:.2f}"]))
    _write_lines(
        f"newton_history_{tag}.csv",
        "iter,f_phi_max,f_gamma_max,f_bl_max,merit,mu,lam,p_fbl,"
        "p_merit,ds_change,termination,wall_s", lines)


def _polish(pack, x0, tag):
    """One GV5.1b-machinery polish from x0 (the pre-registered flags
    verbatim); returns (res_n, wall_s)."""
    print(f"    polish [{tag}]: newton_tight(scaling=rowcol, "
          "lm_damping, floor_stop; max_iter=10) ---", flush=True)
    t0 = time.perf_counter()
    res_n = td.newton_tight(pack, x0=x0, max_iter=10, tol=1e-8,
                            tol_abs=1e-10, line_search=True, verbose=True,
                            scaling="rowcol", lm_damping=True,
                            floor_stop=True)
    wall = time.perf_counter() - t0
    print(f"    polish [{tag}] in {wall:.0f}s: converged="
          f"{res_n['converged']} termination={res_n['termination']} "
          f"n_iter={res_n['n_iter']}", flush=True)
    _write_history(tag, res_n["history"], FLOOR_BAND[tag.split("_")[0]],
                   res_n["termination"])
    return res_n, wall


# ---------------------------------------------------------------------------
# the legs
# ---------------------------------------------------------------------------


def run_leg(level):
    t_leg = time.perf_counter()
    band = FLOOR_BAND[level]
    print(f"=== GV5.1c leg [{level}]: above-band seed polish (floor band "
          f"{band:.2e}; T1 {TARGETS['T1']} / T2 {TARGETS['T2']}) ===",
          flush=True)

    st = gb.build_loose_state(level)   # the amended seed + wiring guard
    res = st["loose_res"]
    loose = gb._loose_committed(level)

    print(f"--- [{level}] tight pack ---", flush=True)
    t0 = time.perf_counter()
    pack = td.build_tight_pack(st, UPWIND_C, M_CRIT, M_CAP, RHO_FLOOR)
    print(f"    pack in {time.perf_counter() - t0:.0f}s", flush=True)

    x_base = pack.x_base()
    fbl_base = fbl_of(pack, x_base)
    floor = gb._gv51_seed_ibl_floor(level)
    print(f"    unperturbed seed F_BL={fbl_base:.3e} = "
          f"{fbl_base / floor:.2f}x the diagnosed floor {floor:.3e} "
          f"(the GV5.1b constructional read: inside the band {band:.2e})",
          flush=True)

    inflow_mask = np.asarray(st["inflow_mask"], dtype=bool)

    def fbl_eps(eps):
        return fbl_of(pack, perturb_delta(
            x_base, pack.n_free, pack.n_st, inflow_mask, eps))

    # -- T1 calibration ------------------------------------------------
    print(f"--- [{level}] T1 seed calibration (target window "
          f"{TARGETS['T1']}) ---", flush=True)
    t0 = time.perf_counter()
    eps1, f1, trace1, ok1, why1 = calibrate_eps(fbl_eps, TARGETS["T1"])
    calib_wall = time.perf_counter() - t0
    _write_lines(
        f"seed_calibration_{level}.csv",
        "seed,eval,eps,f_bl_max,window_lo,window_hi",
        [f"T1,{k},{eps:.6e},{f:.6e},{TARGETS['T1'][0]:.1e},"
         f"{TARGETS['T1'][1]:.1e}" for k, eps, f in trace1])
    print(f"    T1 calibration in {calib_wall:.0f}s "
          f"({len(trace1)} evals): {why1}", flush=True)
    if not ok1:
        _record("calib", level,
                f"T1 calibration FAILED: {why1} -- the pre-registered "
                f"abort: leg stopped, user adjudication | unperturbed "
                f"seed F_BL={fbl_base:.3e} ({fbl_base / floor:.2f}x "
                f"floor)", "RECORDED")
        for b in ("(a)", "(b)", "(c)"):
            _record(b, level, "not reached (calibration abort)",
                    "RECORDED")
        return
    _record("calib", level,
            f"T1 eps={eps1:.3e} -> seed F_BL={f1:.3e} in "
            f"[{TARGETS['T1'][0]:.0e},{TARGETS['T1'][1]:.0e}] "
            f"({f1 / band:.2e}x the floor band; {len(trace1)} evals) | "
            f"unperturbed seed F_BL={fbl_base:.3e} "
            f"({fbl_base / floor:.2f}x floor)", "RECORDED")

    x1 = perturb_delta(x_base, pack.n_free, pack.n_st, inflow_mask, eps1)

    # -- band (a) ------------------------------------------------------
    e1, e2, e3, tol_e2, a_ok = band_a_live_at(pack, x1)
    _record("(a)", level,
            f"live identities on the perturbed T1-seed J: e1={e1:.3e} "
            f"e2={e2:.3e} (<= {tol_e2:.1e} cond-aware, PRE-REGISTERED) "
            f"e3={e3:.3e} (the suite gates the machine-precision "
            "algebra + the mu schedule)",
            "PASS" if a_ok else "FAIL")

    # -- T1 polish -----------------------------------------------------
    res1, wall1 = _polish(pack, x1, level)
    hist1 = res1["history"]
    e_seq1 = [float(h["block_max"][2]) for h in hist1]
    m_seq1 = [float(h["merit"]) for h in hist1]
    triples1 = gb._p_series(e_seq1, band=band)
    converged = bool(res1["converged"])

    # -- T2 escalation (fires only on < 3 triples, not converged) ------
    triples2, fired_t2 = [], False
    eps2 = f2 = None
    res2 = None
    wall2 = 0.0
    if not converged and len(triples1) < 3:
        fired_t2 = True
        print(f"--- [{level}] T2 escalation ({len(triples1)} < 3 "
              f"triples on T1; target window {TARGETS['T2']}) ---",
              flush=True)
        eps2, f2, trace2, ok2, why2 = calibrate_eps(fbl_eps, TARGETS["T2"])
        with open(os.path.join(
                RESULTS, f"seed_calibration_{level}.csv"), "a") as f:
            for k, eps, f in trace2:
                f.write(f"T2,{k},{eps:.6e},{f:.6e},"
                        f"{TARGETS['T2'][0]:.1e},{TARGETS['T2'][1]:.1e}\n")
        print(f"    T2 calibration: {why2}", flush=True)
        if ok2:
            x2 = perturb_delta(x_base, pack.n_free, pack.n_st,
                               inflow_mask, eps2)
            res2, wall2 = _polish(pack, x2, f"{level}_t2")
            e_seq2 = [float(h["block_max"][2]) for h in res2["history"]]
            m_seq2 = [float(h["merit"]) for h in res2["history"]]
            triples2 = gb._p_series(e_seq2, band=band)
            converged = converged or bool(res2["converged"])
        else:
            _record("calib", f"{level}-T2",
                    f"T2 calibration FAILED: {why2} (T1 read stands)",
                    "RECORDED")
    pooled = list(triples1) + list(triples2)

    # -- band (b) ------------------------------------------------------
    verdict_b, key_b = window_verdict(pooled, converged, level)
    p_str = ("; ".join(f"i{i}:{p:.2f}" for i, p in triples1)
             if triples1 else "n/a (no above-band triple on T1)")
    if triples2:
        p_str += (" | T2: " + "; ".join(f"i{i}:{p:.2f}"
                                        for i, p in triples2))
    pm_str = ("; ".join(f"i{i}:{p:.2f}"
                        for i, p in gb._p_series(m_seq1))
              or "n/a")
    slope, n_pairs = regression_slope(e_seq1, band)
    cf = contraction_factors(e_seq1, band)
    cf_str = "; ".join(f"i{i}:{d:.2f}dex" for i, d in cf) or "n/a"
    slope_str = (f"{slope:.2f} over {n_pairs} pairs"
                 if slope is not None else f"n/a ({n_pairs} pairs)")
    fin = hist1[-1]
    fin_fbl, fin_merit = float(fin["block_max"][2]), float(fin["merit"])
    g51b_fbl, g51b_merit = _gv51b_final(level)
    key_b_full = (
        f"{key_b} | T1 F_BL p (above-band): {p_str} | merit p (T1): "
        f"{pm_str} | contraction factors (T1): {cf_str} | regression "
        f"slope (T1): {slope_str} | T1 final F_BL {fin_fbl:.3e} vs "
        f"floor {floor:.3e} ({fin_fbl / floor:.2f}x), final merit "
        f"{fin_merit:.3e} vs GV5.1b committed {g51b_merit:.3e}, "
        f"termination={res1['termination']}")
    _record("(b)", level, key_b_full, verdict_b)

    # -- band (c) ------------------------------------------------------
    n_to_band = next((i for i, v in enumerate(e_seq1) if v <= band), None)
    n_polish = res1["n_iter"] + (res2["n_iter"] if res2 else 0)
    n_total = res.n_outer + n_polish
    asp = 2 * loose["n_outer"]
    key_c = (f"N_polish={n_polish} (T1 {res1['n_iter']} term="
             f"{res1['termination']}"
             + (f" + T2 {res2['n_iter']} term={res2['termination']}"
                if res2 else "")
             + f"; band-entry iter={'none' if n_to_band is None else n_to_band}"
             f" -- the seed F_BL {e_seq1[0]:.3e} sits ABOVE the band "
             f"{band:.2e} by construction) | N_total={n_total} (regen "
             f"loose {res.n_outer} + {n_polish}) vs committed loose "
             f"{loose['n_outer']} | aspirational N_polish <= {asp}: "
             f"{'met' if n_polish <= asp else 'NOT met'}"
             + (" (3+: recorded; user adjudication)" if n_polish >= 3
                else "")
             + " | deliberately far seeds: N_polish measures the window "
             "traversal, not production convergence")
    _record("(c)", level, key_c, "RECORDED")

    # -- compare.csv ---------------------------------------------------
    retries = sum(h.get("mu_retries", 0) for h in hist1)
    mus = [h["mu"] for h in hist1 if h.get("mu") is not None]
    cmp_rows = [
        ("unperturbed seed |F_BL|_max (x the diagnosed floor)",
         f"{fbl_base:.6e} ({fbl_base / floor:.2f}x)", f"floor {floor:.3e}",
         "the GV5.1b constructional read (inside the band)"),
        ("T1 eps / seed |F_BL|_max", f"{eps1:.6e} / {f1:.6e}",
         f"window [{TARGETS['T1'][0]:.0e},{TARGETS['T1'][1]:.0e}]",
         f"{f1 / band:.2e}x the floor band; {len(trace1)} calibration "
         "evals"),
        ("band_a_e1 diag(rn)@Jsc@diag(cn) vs J (relerr)",
         f"{e1:.3e}", f"<= {BAND_A_TOL[0]:.0e}",
         "on the perturbed T1-seed J"),
        ("band_a_e2 mu=0 damped vs undamped splu (relerr)",
         f"{e2:.3e}", f"<= {tol_e2:.1e}",
         "cond-aware tolerance PRE-REGISTERED (the GV5.1b adjudication)"),
        ("band_a_e3 dx = C dy round-trip (relerr)",
         f"{e3:.3e}", f"<= {BAND_A_TOL[2]:.0e}", ""),
        ("T1 F_BL p above the floor band", p_str, "",
         "the band-(b) read"),
        ("T1 merit p (all contractions)", pm_str, "",
         "recorded alongside per the prereg"),
        ("T1 contraction factors log10(e_prev/e_i)", cf_str, "",
         "fallback context"),
        ("T1 regression slope log e_{i+1} vs log e_i", slope_str, "",
         "fallback context (>= 3 pairs)"),
        ("above-band triples T1 / pooled", f"{len(triples1)} / "
         f"{len(pooled)}", ">= 3 -> median-p band applies",
         f"T2 escalation fired: {fired_t2}"
         + (f" (eps={eps2:.3e}, seed F_BL={f2:.3e})" if res2 else "")),
        ("T1 final |F_BL|_max vs floor", f"{fin_fbl:.6e}",
         f"{g51b_fbl:.6e}",
         f"vs GV5.1b committed final; {fin_fbl / floor:.2f}x the "
         f"diagnosed floor"),
        ("T1 final merit", f"{fin_merit:.6e}", f"{g51b_merit:.6e}",
         "vs GV5.1b committed final"),
        ("T1 termination / converged",
         f"{res1['termination']} / {res1['converged']}",
         "GV5.1b: floor_reached (medium) / cap (coarse)", ""),
        ("N_polish (T1[+T2]) / N_total vs committed loose outer",
         f"{n_polish} / {n_total}", loose["n_outer"],
         f"regen loose n_outer={res.n_outer}"),
        ("mu schedule: final mu / total reject-retries (T1)",
         f"{mus[-1]:.3e} / {retries}" if mus else "n/a", "", ""),
        ("loose committed: cl / ds_max / ibl_floor",
         "", f"{loose['cl']:.8f} / {loose['ds_max']:.6e} / "
         f"{loose['ibl_floor']:.3e}", "read; never recomputed"),
        ("polish wall time (s) T1[+T2]",
         f"{wall1:.0f}+{wall2:.0f}" if fired_t2 else f"{wall1:.0f}", "",
         f"threads={THREADS} (temporary 8-thread session constraint; "
         "runner default 16)"),
    ]
    _merge_compare(level, cmp_rows)
    _record("wall", level,
            f"leg total {time.perf_counter() - t_leg:.0f}s (loose regen "
            f"+ pack + calibration + band (a) + polish[{'+T2' if fired_t2 else ''}]) "
            f"| threads={THREADS} (temporary 8-thread session "
            "constraint; runner default 16)", "RECORDED")
    print(f"GV5.1c [{level}] done in "
          f"{time.perf_counter() - t_leg:.0f}s", flush=True)


def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--levels", nargs="+", default=["coarse", "medium"],
                    choices=["coarse", "medium"],
                    help="mesh level(s) to run (default: both)")
    args = ap.parse_args()

    print(f"GV5.1c runner: threads={THREADS} (temporary 8-thread "
          "session constraint; runner default 16)", flush=True)
    for level in args.levels:
        run_leg(level)

    n_fail = sum(1 for *_, v in SUMMARY if v == "FAIL")
    n_pass = sum(1 for *_, v in SUMMARY if v == "PASS")
    n_rec = sum(1 for *_, v in SUMMARY if v == "RECORDED")
    print(f"\nGV5.1c: {n_pass} PASS / {n_fail} FAIL / {n_rec} RECORDED",
          flush=True)
    if n_fail:
        print("HONEST FAIL -- see summary.csv", flush=True)
        sys.exit(1)


if __name__ == "__main__":
    main()
