...
## 6. 结论与后续（第 3/4 项用户已裁决 2026-07-20 → B29 执行）
...
3. **哪个片位置更物理**（平 y=0 顺 conforming 近似 vs 斜随来流）——
   ...
   **用户裁决：采纳 flat-fragment 为翼身 LS 生产配置 → B29 执行**
   （B18 demo LS 生产腿 = clip + 平片重解、GB18.5/GB18.4 C 侧记录
   实况化重锚、M0.5 锚 0.2087/0.2117→0.2115/0.2184、b9 LS 接线测试
   切生产配置；A 侧斜片保留为历史对照）。
4. B29 下的预期：B18 demo 的 LS 腿、B26 的 (b) 类天花板归因（翼尖
   P13 + 高 M Newton）不受影响（片位置不碰翼面解），但 GB18.5 的
   cl_fus 记录值会改（实况分解取代 B26 斜片引用值）。
   **B29 实测（committed B18 checks.csv，8/8 PASS）**：预期成立——
   medium 天花板 0.7625→**0.775**（死 0.7875，a+dm；垂死峰 M3.98 @
   翼尖 z=1.20 实况，址不变、P13 坐实）；跨模型 gap 2.6→**0.5%**
   （M0.5）、2.4→**1.1%**（M0.65）、2.5→**1.1%**（M0.75）；GB18.5
   实况平片分解 cl_fus **0.0382**（带 −0.0006 / 带外 0.0388 / 极
   0.0007）@0.7875 vs conf 0.0423 @0.79——片位置不碰翼面解，cl_fus
   记录值如期重锚。
