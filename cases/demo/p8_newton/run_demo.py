"""
P8 demo -- fully-coupled (phi_red, Gamma) Newton (gates G8.1; G8.2/G8.3
are the N6 M6-performance work package, not shown here).
...
(user-approved) to coarse M0.80 + medium M0.7875.
  ...
  part 2 (PYFP3D_TRANSONIC_GATES=1, ~5 min): medium M0.7875 gate run --
      V8.1b convergence + per-stage runtime breakdown (V8.2-lite; the M6
      breakdown lands with N6).
...
def part1(cl: CheckList):
    print("\n[1/2] subsonic agreement + coarse M0.80 Newton continuation")
...
def part2(cl: CheckList):
    print("\n[2/2] medium M0.7875 G8.1 gate run [heavy]")
...
def main():
    apply_style()
    cl = CheckList("P8 fully-coupled Newton (G8.1; G8.2/G8.3 = N6)")
    part1(cl)
    if os.environ.get("PYFP3D_TRANSONIC_GATES", "0") == "1":
        part2(cl)
    else:
        print("\n[2/2] medium gate run skipped (~5 min); "
